package com.zh.learn.param_transfer_test;
...
public class ValueTest {
    ...
    private static void pass(int j) {
        ...
        j = 20;//基本类型，实参传给函数的是原实参的一个副本，改动不影响主方法原值
        System.out.println("in pass, i = " + j);
    }

    private static void pass(User user) {

        user = new User();//传递原对象堆里的引用，但是关键在于有new操作，指向另一块新对象引用地址，操作的地址不是原来的地址
        user.setName("fang");
        user.setAge(20);
    ...
    private static void pass(String name) {
        name = "song";//传递原对象堆里的引用，但是对于string类型操作先new，本质还是指向新内存地址，操作不改变原对象
        System.out.println("in pass, name = "+name);
    }

    private static void pass1(User user) {
        //传递原对象堆里的引用，但是副本和正本的引用指向同一个地址，set操作将改变同一个地址下的属性值，所以main方法中的对象属性已经发生改变了
        user.setName("fang");
        user.setAge(20);
        System.out.println("in pass, user = " + user.toString());
    }
